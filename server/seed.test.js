...
describe('fresh schema and development fixtures', () => {
    it('authenticates the exact fixture accounts and returns one shared catalog', async () => {
        ...
        let server;
        try {
            await db.initDatabase({ seedDevData: true });
    ...
    it('preserves edits, deletions, and installation identity across reopening', async () => {
        ...
        try {
            ...
            await db.runSql("UPDATE users SET passwordHash = ?, isAdmin = 0 WHERE username = 'admin'", [hashPassword('changedpassword')]);
            await db.runSql("DELETE FROM users WHERE username = 'user'");
            ...
            await db.initDatabase({ seedDevData: true });
            await createApp({ database: db, adminUsername: 'admin' }).bootstrapAdmin();
    ...
    it('refuses fixture injection into an initialized empty installation and rolls back legacy initialization', async () => {
        const db = openDatabase(':memory:');
        ...
        try {
            ...
            await legacy.runSql('CREATE TABLE users (id INTEGER PRIMARY KEY)');
            await expect(legacy.initDatabase({ seedDevData: true })).rejects.toThrow('database_reset_required');
            expect(await legacy.allSql("SELECT name FROM sqlite_master WHERE type = 'table'")).toEqual([{ name: 'users' }]);
        } finally { await db.close(); await legacy.close(); }
    });
