...
export async function initializeSchema(tx, { seedDevData = false, fixtureCredentials } = {}) {
    ...
    await tx.runSql('CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, userId INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, expiresAt INTEGER NOT NULL)');
    ...
    await tx.runSql(`CREATE TABLE IF NOT EXISTS workouts (
        id TEXT PRIMARY KEY NOT NULL CHECK (${uuidCheck('id')}), userId INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    ...
    END`);
    for (const [table, columns] of [['workouts', 'gymId, startTime, endTime'], ['userMeasurements', 'weight, bodyFat, timestamp']]) {
