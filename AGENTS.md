...
- Translations: `src/i18n/translations.ts` re-exports `Language`, `TranslationKey`, and the `translations` map; strings live per language in `src/i18n/locales/en.ts` (defines the key set) and `src/i18n/locales/de.ts` (typed against it, so a missing or extra German key fails `tsc`). Add a new key to both files.
