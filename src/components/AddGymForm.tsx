    ...
    return (
        <form onSubmit={handleSubmit} className="p-5 bg-[var(--card)] rounded-2xl border border-[var(--border)] shadow-xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-[var(--foreground)]">{t('addGym.title')}</h3>
            ...
            <div className="space-y-4">
                <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-2">{t('addGym.nameLabel')}</label>
                    <input
                        type="text"
                        value={name}
                ...
                <div>
                    <label className="block text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider mb-2">{t('addGym.locationLabel')} <span className="text-[var(--muted-foreground)] font-normal normal-case">({t('common.optional')})</span></label>
            ...
            <div className="flex gap-3 justify-end mt-8">
                <button type="button" onClick={onCancel} className="px-4 py-2 text-[var(--muted-foreground)] hover:text-[var(--foreground)] font-medium transition-colors">{t('addGym.cancel')}</button>
                <button
                    type="submit"
