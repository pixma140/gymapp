...
Status: ready for implementation; no application changes or database wipes are performed by writing this plan.
...
- [ ] Record the current passing test/build/lint baseline and inventory all domain writes, route ID parsing, table consumers, and exercise references.
- [ ] Extract `createApp` and database lifecycle without changing existing behavior first. Adapt HTTP tests to inject and close temporary databases explicitly.
- [ ] Add a serialized transaction helper: asynchronous statements inside one transaction must not interleave with another request's writes on the shared SQLite connection.
